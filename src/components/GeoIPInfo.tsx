    ...
    <Card className="bg-card border-border"> {/* Updated background and border */}
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-foreground"> {/* Updated text color */}
          <MapPin className="h-5 w-5 text-purple-500 dark:text-purple-400" /> {/* Updated text color */}
          GeoIP Information
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-3">
            {leftFields.map((f, index) => (
              <div key={index} className="bg-muted rounded-lg p-3"> {/* Updated background */}
                <p className="text-sm text-muted-foreground mb-1">{f.label}</p> {/* Updated text color */}
                <p className="text-foreground font-mono flex items-center gap-2"> {/* Updated text color */}
                  {f.icon && <f.icon className="h-4 w-4 text-muted-foreground/70" />} {/* Updated text color */}
