  ...
  if (!lfi || !lfi.tested) {
    return (
      <Card className="bg-card border-border"> {/* Updated background and border */}
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-orange-500 dark:text-orange-400"> {/* Updated text color */}
            <FileWarning className="h-5 w-5" />
            LFI Vulnerability Assessment
          </CardTitle>
          <CardDescription className="text-muted-foreground">Local File Inclusion vulnerability testing not performed</CardDescription> {/* Updated text color */}
        </CardHeader>
      </Card>
    );
  }
        ...
        {lfi.vulnerable && lfi.vulnerabilities.length > 0 ? (
          <>
            <Alert variant="destructive" className="bg-destructive/10 border-destructive/50"> {/* Updated background and border */}
              ...
              <AlertDescription className="font-semibold text-destructive dark:text-red-400"> {/* Updated text color */}
                {lfi.vulnerabilities.length} Local File Inclusion vulnerabilit{lfi.vulnerabilities.length === 1 ? 'y' : 'ies'} detected!
              </AlertDescription>
            </Alert>

            <Alert variant="destructive" className="bg-destructive/10 border-destructive/50 mt-4"> {/* Updated background and border */}
            ...
            <div className="space-y-4">
              {lfi.vulnerabilities.map((vuln, index) => (
                <div key={index} className="border border-border rounded-lg p-4 bg-muted"> {/* Updated background and border */}
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Badge className={getSeverityColor(vuln.severity)}>
                        {vuln.severity.toUpperCase()}
                      </Badge>
                      <span className="text-foreground font-medium">{vuln.type}</span> {/* Updated text color */}
                    </div>
                    <Badge variant="outline" className="text-xs text-muted-foreground border-border"> {/* Updated text and border color */}
                  ...
                  {vuln.parameter && (
                    <div className="mb-2">
                      <span className="text-xs text-muted-foreground">Parameter: </span> {/* Updated text color */}
                      <code className="text-xs bg-muted/50 px-1.5 py-0.5 rounded text-foreground">{vuln.parameter}</code> {/* Updated background and text color */}
                    </div>
                  )}

                  <div className="mb-2">
                    <span className="text-xs text-muted-foreground block mb-1">Payload:</span> {/* Updated text color */}
                    <code className="text-xs bg-muted/50 px-2 py-1 rounded block overflow-x-auto text-foreground"> {/* Updated background and text color */}
                      {vuln.payload}
                    </code>
                  </div>

                  <div className="mb-2">
                    <span className="text-xs text-muted-foreground block mb-1">Indicator:</span> {/* Updated text color */}
                    <div className="text-amber-600 dark:text-amber-400 font-mono"> {/* Updated text color */}
                      {vuln.indicator}
                    </div>
                  </div>
                  ...
                  {vuln.evidence && (
                    <div>
                      <span className="text-xs text-muted-foreground block mb-1">Evidence:</span> {/* Updated text color */}
                      <pre className="text-xs bg-muted/50 p-2 rounded overflow-x-auto max-h-32 overflow-y-auto text-foreground"> {/* Updated background and text color */}
                        {vuln.evidence}
                      </pre>
                    </div>
                  )}
          ...
          <Alert className="bg-green-500/10 border-green-500/50"> {/* Updated background and border */}
            <CheckCircle2 className="h-4 w-4 text-green-500 dark:text-green-400" /> {/* Updated text color */}
            <AlertDescription className="text-green-600 dark:text-green-400"> {/* Updated text color */}
              No Local File Inclusion vulnerabilities detected
            </AlertDescription>
          </Alert>
        )}

        <div className="pt-2 border-t border-border"> {/* Updated border */}
