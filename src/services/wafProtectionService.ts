...
const WAF_PROTECTION_INDICATORS = {
  HEADERS: {
    'server': [
...
export const performWAFProtectionCheck = async (
  target: string,
  numRequests: number = 20,
  intervalMs: number = 100, // Interval between requests
  requestManager?: RequestManager
): Promise<WAFProtectionResult> => {
  console.log(`[WAF Protection Check] Starting for ${target} with ${numRequests} requests`);

  const result: WAFProtectionResult = {
    tested: true,
  ...
  try {
    ...
    for (let i = 0; i < numRequests; i++) {
      ...
      try {
        ...
        for (const headerName in WAF_PROTECTION_INDICATORS.HEADERS) {
          const headerValue = response.headers.get(headerName);
          if (headerValue) {
            const patterns = (WAF_PROTECTION_INDICATORS.HEADERS as any)[headerName];
        ...
        for (const pattern of WAF_PROTECTION_INDICATORS.BODY_PATTERNS) {
        ...
        console.warn(`[WAF Protection Check] Request failed: ${error.message}`);
    ...
    for (const status in statusCounts) {
      ...
      if (WAF_PROTECTION_INDICATORS.STATUS_CODES.includes(parseInt(status))) {
        result.firewallDetected = true;
    ...
    console.log(`[WAF Protection Check] Complete. Firewall detected: ${result.firewallDetected}`);
    return result;
  } catch (error: any) {
    if (error.message === 'Request aborted') {
      throw error;
    }
    console.error('[WAF Protection Check] Critical error:', error);
    return {
      ...result,
      tested: false,
      firewallDetected: false,
...
export type DDoSFirewallResult = WAFProtectionResult;
export const performDDoSFirewallTest = performWAFProtectionCheck;
